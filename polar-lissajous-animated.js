...
const P_start = 2.45;                     // Frequency of the radius pulse (p)
const Q_start = 1.756;                     // Frequency of the angle sweep (q)

const P_end = 4.7876;                     // Frequency of the radius pulse (p)
const Q_end = 6.34;                     // Frequency of the angle sweep (q)
...
for (let frame = 0; frame < frames; frame++) {
  const P = P_start + (P_end - P_start) * (frame / (frames - 1));
  const Q = Q_start + (Q_end - Q_start) * (frame / (frames - 1));
